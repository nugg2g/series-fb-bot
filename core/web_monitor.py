    ...
    def _dump_to_disk(self):
        try:
            ...
            data = {
                ...
                "delay_total_seconds": self.delay_total_seconds,
                ...
                if os.path.exists(cfg_path):
                    with open(cfg_path, "r", encoding="utf-8") as f:
                        ...
                        config_summary = {
                            ...
                            "tags_count": cfg.get("tags_count", 10),
                            "hashtag_pool": cfg.get("hashtag_pool", []),
                            "ai_caption_enabled": cfg.get("ai_caption", {}).get("enabled", True),
                            "ai_caption_model": cfg.get("ai_caption", {}).get("model", "gemini-3.5-flash-lite"),
                            "ai_caption_api_key": cfg.get("ai_caption", {}).get("api_key", ""),
                            "mark_as_ai_content": cfg.get("mark_as_ai_content", True),
                            "strict_page_guard": cfg.get("strict_page_guard", True),
                            "auto_watch_new_files": cfg.get("auto_watch_new_files", True),
                            "cta_post_enabled": cfg.get("cta_post_enabled", True),
                            "cta_post_interval_reels": cfg.get("cta_post_interval_reels", 5)
                        }
            except Exception:
                pass
    ...
    def _loop(self):
        ...
        while self.running:
            try:
                ...
                r = requests.post(sync_endpoint, json=payload, headers=headers, timeout=25)
                if r.status_code == 200:
                    if not connected_logged:
                        safe_print(f"✅ [CloudSync] Live connection established with Cloud Relay ({self.cloud_url})")
